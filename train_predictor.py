...
scheduler = lr_scheduler.ExponentialLR(optimizer, args.anneal_rate) #TODO: necessary?
...
for epoch in range(args.epoch):
    dataset_x = DataFolder(args.train, args.batch_size)
    dataset_y

    for batch_x,batch_y in zip(tqdm(dataset_x),tqdm(dataset_y)):
        total_step += 1
        model.zero_grad()
        y_pred = model.forward_predict(*batch, beta=beta) #TODO: change loss to be Categorical cross entropy loss (multiclass) or binary crossentropy loss
        y_true = batch_y
        ...
        if total_step % args.print_iter == 0:
            ...
            print("[%d] Beta: %.3f, loss: %.3f" % (total_step, beta, meters[0], param_norm(model), grad_norm(model)))
            sys.stdout.flush()
            meters *= 0

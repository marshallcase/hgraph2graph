...
class HierMPNDecoder(nn.Module):
    ...
    def decode(self, src_mol_vecs, greedy=True, max_decode_step=None, beam=5,max_AA=15,max_nodes=None, max_edges=None, max_sub_nodes=None):
        ...
        if max_AA is None:
            max_AA = self.max_AA
        if max_decode_step is None:
            max_decode_step = self.max_decode_step
            
        src_root_vecs, src_tree_vecs, src_graph_vecs = src_mol_vecs
        batch_size = len(src_root_vecs)

        tree_batch = IncTree(batch_size, node_fdim=2, edge_fdim=3,max_nodes=self.max_nodes,max_edges=self.max_edges,max_sub_nodes=self.max_sub_nodes)
